...
export function externalLink(label: string, url: string): HTMLElement {
    ...
    } catch { return element('span', 'muted', '유효하지 않은 출처 링크'); }
}
export function math(tex: string, display = false): HTMLElement {
    ...
    try {
        if (tex.length > 12000) throw new Error('수식 길이 초과');
        ...
        node.classList.add('formula-error'); node.append(element('code', '', tex));
        node.title = '수식을 표시하지 못했습니다. 원본 표기를 보존합니다.';
        if (display) node.append(element('small', '', '수식 표시 오류 · 원본 표기를 보존했습니다.'));
    }
    return node;
}
export function inline(text: string, depth = 0): DocumentFragment {
    const out = document.createDocumentFragment();
    if (depth > 5) { out.append(document.createTextNode(text)); return out; }
    const pattern = /(\$\$[\s\S]+?\$\$|`[^`\n]+`|\*\*[\s\S]+?\*\*|\$[^$\n]+\$)/g;
    let cursor = 0;
    for (const match of text.matchAll(pattern)) {
        ...
        const token = match[0];
        if (token.startsWith('$$')) out.append(math(token.slice(2, -2), true));
        else if (token.startsWith('$')) out.append(math(token.slice(1, -1)));
        else if (token.startsWith('`')) out.append(element('code', 'inline-code', token.slice(1, -1)));
        else { const strong = element('strong'); strong.append(inline(token.slice(2, -2), depth + 1)); out.append(strong); }
        cursor = i + token.length;
    }
    out.append(document.createTextNode(text.slice(cursor))); return out;
}
function appendGroup(node: HTMLElement, group: string): void {
    ...
    for (const line of group.split('\n')) {
        ...
        if (m) {
            ...
            if (!list || list.tagName !== tag) {
                ...
                if (list instanceof HTMLOListElement) list.start = Number(m[2]);
                node.append(list);
            }
            const li = element('li'); li.append(inline(m[3])); list.append(li); paragraph = null;
        } else if (list && /^\s{2,}\S/.test(line)) {
            list.lastElementChild?.append(document.createElement('br'), inline(line.trim()));
        } else {
            list = null;
            if (!paragraph) { paragraph = element('p'); node.append(paragraph); }
            else paragraph.append(document.createElement('br'));
            paragraph.append(inline(line.replace(/^>\s?/, '')));
            if (line.startsWith('>')) paragraph.classList.add('quote');
        }
    }
}
export function prose(text: string): HTMLElement {
    const node = element('div', 'prose');
    // Preserve multiline display math before prose/list line splitting.
    for (const chunk of text.split(/(\$\$[\s\S]+?\$\$)/g)) {
        if (chunk.startsWith('$$') && chunk.endsWith('$$')) {
            node.append(math(chunk.slice(2, -2), true));
        } else {
            for (const group of chunk.split(/\n\s*\n/).filter(x => x.trim())) appendGroup(node, group);
        }
    }
    return node;
}
