 ...
 * Runs against a temporary copy of dist; original build output is never mutated. */
...
const temporary=await fs.mkdtemp(path.join(os.tmpdir(),'study-native-smoke-'));
...
try{
 await fs.cp(path.resolve('dist'),root,{recursive:true});
 ...
 await check('source text search and navigation',async()=>{await page.keyboard.press('Control+k');await page.locator('.search-input').fill('검사전용검색어');await page.locator('.search-result').first().waitFor();const hits=await page.locator('.search-results').innerText();assert.match(hits,/통합 검사 수업/);await page.getByRole('button',{name:'닫기',exact:true}).click();});
...
}finally{await browser?.close();if(server)await new Promise(r=>server.close(r));await fs.rm(temporary,{recursive:true,force:true});}
